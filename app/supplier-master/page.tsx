...
export default function SupplierMaster() {
  ...
  useEffect(() => {
    const synchronizeSupplierKeys = () => {
      try {
        const rawData = localStorage.getItem("supplierMaster");
        if (!rawData) return;
        ...
        if (dataModified) {
          localStorage.setItem("supplierMaster", JSON.stringify(fixedCollection));
        }
      } catch (err) {
        console.error("Master data unification intercept exception:", err);
      } finally {
        setSynced(true);
      }
    };

    synchronizeSupplierKeys();
  }, []);

  // Global Escape Key Intercept to trigger Clear Action
  useEffect(() => {
    const handleGlobalEscapeKey = (e: KeyboardEvent) => {
      ...
      const clearButton =
        Array.from(document.querySelectorAll<HTMLButtonElement>("button")).find(
          (el) => el.textContent?.toLowerCase().includes("clear")
        ) || null;
  ...
  // Keyboard Intercept Navigation Engine Loop (Fixed Bracket Closure)
  useEffect(() => {
  ...
  if (!synced) {
    return (
      <div style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "radial-gradient(circle at top left, rgba(255,255,255,0.98) 0%, rgba(255,248,220,0.94) 34%, rgba(245,224,148,0.86) 70%, rgba(214,188,95,0.82) 100%)", color: "#4a3500" }}>
        <div style={{ fontWeight: 800, fontSize: "15px", letterSpacing: "0.5px" }}>Synchronizing Supplier Master Ledger Properties...</div>
      </div>
    );
  }

  return <PartyMasterPage MASTER_TYPE="supplier" />;
}
